...
def load_prob_aprobar(conn):
    cols = table_cols(conn, "prob_aprobar_ctx")
...
def main():
    ...
    out_csv = OUT_DIR / f"demanda_simulada_rf_{PERIODO_OBJ}.csv"
    df_out.to_csv(out_csv, index=False, encoding="utf-8")
    print(f"CSV generado: {out_csv}")

    table = f"demanda_simulada_rf_{PERIODO_OBJ}"
